...
type SubState = {
  data: AdminPermissionType[],
  showModal: (values: AdminPermissionType) => void,
    ...
    },
    {
      ...
      options: {
        customBodyRender: (value: any) => {
          ...
          return (
            <>
              {value && value.length > 0 && value.map((role: AdminRoleType) => {
                let chipIcon;
                const verb = role.name.split(' ')[0];
                switch (verb) {
                  default:
                  case 'view':
                    chipIcon = eyeOutline;
                ...
                return (
                  <IonChip style={chipStyle}>
                    ...
                    <IonLabel color="black">{role.name}</IonLabel>
                  </IonChip>
                );
              })}
            </>
